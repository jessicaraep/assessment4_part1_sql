...
SELECT m.name, brand_name, headquarters FROM models AS m
JOIN brands AS b
...
SELECT m.name, brand_name, founded FROM models AS m
LEFT JOIN brands AS b
ON b.name = m.brand_name WHERE year = 1960;
...
SELECT b.name, m.name, year, discontinued, discontinued
- year AS years_until_brand_discontinued
FROM models AS m
JOIN brands AS b
ON m.brand_name = b.name
WHERE discontinued IS NOT NULL
ORDER BY b.name, m.name, year;
